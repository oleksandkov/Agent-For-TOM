...
- **8 MCP-серверiв за замовчуванням, i жоден не потребує ключа** — `playwright`, `context7`, `word-docs`, `sequential-thinking`, `fetch`, `time`, `excel`, `pdf` (злиття/розбиття/OCR/таблицi/форми для готових PDF — `read_file` й так витягує текст, це решта роботи з PDF). `TOMAS setup` пiсля встановлення реально пiдключається до кожного i показує кiлькiсть iнструментiв — «налаштовано» й «працює» не одне й те саме. Сервер, що потребує ключа (пошук, GitHub…), свiдомо не входить у типовий набiр — додайте його самi через `TOMAS mcp add`, коли ключ буде.
