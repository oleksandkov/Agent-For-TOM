...
def render(contract_path: str, content_plan_path: str, output_path: str) -> None:
    ...
    # Signed, so a later run can tell this apart from an original if it is
    # ever handed back as a "sample". One measured session took its own
    # previous output as the reference and inherited every defect in it;
    # `run.py` looks for exactly this string. Written to `comments` because
    # `author` is python-docx's and overwriting it would lose the weaker
    # signal that recognises outputs made before this existed.
    doc.core_properties.comments = "generated by document-style-match"
