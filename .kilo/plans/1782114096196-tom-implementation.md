# TOM — Implementation / План реалізації та організації

Цей документ — технічний план реалізації ідеї, описаної в `tom-idea.md`. Тут: рішення, архітектура, фази, розподіл між двома розробниками, тестування, ризики.

## Мета

Створити локального AI-агента "TOM" з desktop-інтерфейсом, який підтримує багато провайдерів (Ollama + cloud), MCP, має 3-рівневу пам'ять і авто-створення скілів. Побудований як глибокий форк Langflow з embedded Letta SDK і окремим Tauri-фронтендом. Повністю самодостатній інсталятор, офлайн з локальним LLM.
...
| Шляхи даних | Win: `%APPDATA%\tom\`; macOS: `~/Library/Application Support/tom/`; Linux: `~/.local/share/tom/` | Platform-стандарт, один каталог = бекап |
| Ліцензія | MIT | Сумісно з Langflow і Letta |
...
## Організація роботи: команда з 2-х

- **Backend-розробник** (ви) — backend-шар, БД, пам'ять, MCP-сервери, PyInstaller
- **Frontend-розробник** — Tauri-UI, React-компоненти, UX, Tauri bundle

Паралелізація: кожна фаза має **B-substep + F-substep**, що йдуть паралельно після фіксації контракту.
...
Розмір — T-shirt (S/M/L/XL), не години/дні.

| # | Phase | Розмір | B-substep (backend) | F-substep (frontend) | Інтеграція (e2e) |
|---|---|---|---|---|---|
